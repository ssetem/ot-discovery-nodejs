...
describe "mock intergration server tests", ->
  ...
  respondToWatchTest = (client, done) ->
    client.onError (err) ->
      done err
  ...
  respondToAnnounceTest = (client, done) ->
    ...
    client.connect (err) ->
      ...
      , (err, resp, something) ->
        ...
        client.announcementIndex.computeDiscoveryServers = () ->
          client.disconnect()
  ...
  respondToUnannounceTest = (client, done) ->
    ...
    client.connect (err) ->
      client.announce
        ...
        serviceUri: "fake://test"
      , (err, resp) ->
        client.unannounce resp, (err, resp) ->
          if err then done(err)
          expect(resp).to.be.ok
          client.announcementIndex.computeDiscoveryServers = () ->
            client.disconnect()
  ...
  describe "api v1 tests", ->
    it "should respond to watch", (done) ->
      respondToWatchTest( new DiscoveryClient('localhost:9000')
        , done)
  ...
  describe "api v2 tests", ->
    it "should respond to watch", (done) ->
      respondToWatchTest( new DiscoveryClient('localhost:9000', ['localhost:9001'], 'homeRegionName', 'test-serviceType')
        , done)
